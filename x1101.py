...
def run_subprocesses_f():
    ...
    if not os.path.exists("x1101"):
        ...
        subprocess.run("install trash-cli", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    progress_done = True
...
commands = [
    ...
    (f"cd {ui_path} && git reset --hard && git pull && git switch {branch} && git pull && git reset --hard && trash-put /opt/conda/lib/python3.10/site-packages/aiohttp*", "Updating UI..."),
